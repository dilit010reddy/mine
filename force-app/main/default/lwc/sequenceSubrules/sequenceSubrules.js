...
import getSubrules from '@salesforce/apex/SubRuleHandler.getSubrules';
...
const columns = [
    ...
    { label: 'Rule Name', fieldName: 'Description__c', hideDefaultActions: true, sortable: false, wrapText: true }
]
export default class SequenceSubrules extends LightningElement {
    @api recordId;
    @track columns;
    isLoading = true;
    error;
    ...
    renderedCallback() {
        if (!this.retrievedRecordId && this.recordId){
            ...
            getSubrules({detailId:this.recordId})
            .then((result)=>{
                //alert(String.fromCharCode(18+64))
                this.dataTable = true;                
                this.rules = result;
                this.isLoading = false;
               // alert(numToSSColumnLetter(5));
            })
            .catch((error)=>{                    
    ...
    saveSequences() {
        // console.log('in save Sequences')
        // alert('inSaveseq')
        const promises = this.rules.map((currentRow) => {
            const fields = { Id: currentRow.Id, Order__c: currentRow.Order__c,Alpha_Order__c :String.fromCharCode(currentRow.Order__c+64)};
            const recordInput = {
                fields: fields
            };
            console.log('save Sequences'+recordInput)
            //alert(JSON.stringify(recordInput))
            updateRecord(recordInput);
        });
        console.log('save Sequences'+promises);
